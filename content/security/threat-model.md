...
| Tampering | An adapter left behind when the Engine follows its Vault to a new generation | The adapter check is a condition of use and not only of registration. `set_vault` moves the Engine's end of a pairing the pool registry cannot be cleared of, so a pool registered before it names a Vault the Engine no longer governs, and the next allocation would release the new Vault's USDC to an adapter that repays the old one. `allocate`, `deallocate` and `recover` re-read the adapter's `engine()` and `vault()` and refuse with `AdapterMismatch` until `set_counterparties` brings it across. |
