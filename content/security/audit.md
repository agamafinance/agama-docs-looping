...
A fourth pass went over the two entry points those changes added, on the rule the earlier rounds established: every new quantity opens a hole wherever it is read. The oracle's round record came out clean. The recovery booking did not, quite, and what it turned up is a disclosure rather than a finding, which is the honest label for it.

`recover` takes a pool and sweeps that pool's adapter, so the cash and the attribution come from the same place and the caller cannot separate them. `book_recovery` also takes a pool, and there the cash is already in the Vault and unattributed by construction, because being unable to say where it came from is the entire reason the call exists. Which pool gets its concentration charge back is therefore something the admin asserts, with nothing on-chain to check it against. The global loss book and the Vault's both move by exactly the cash that arrived whatever pool is named, so solvency does not rest on the assertion; the per-pool concentration cap does, and a recovery booked against the wrong pool frees a limit for a pool whose loss did not come home.
...
Two more seams were closed the same afternoon, both found by applying one question across every contract: which counterparty pointers are interrogated before they are accepted, and which can be read back afterwards.

The Vault's oracle pointer was the only one of its counterparties that was written after checking nothing at all. The token it names, the agUSD it mints and the Engine it authorises are each interrogated; the oracle address did not have to be a contract, a contract did not have to be an oracle, and an oracle did not have to have heard of the feed. It now has to answer for that exact feed, and deliberately nothing more, because pointing at a newly deployed oracle before its first report is an ordinary operation. What no check reaches is a real feed that is the wrong one for the book, which is why both halves of the pair are now readable: the missing getter is what let the missing check go unnoticed.
