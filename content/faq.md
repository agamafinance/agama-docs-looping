...
A Curator, which in V1 is the admin multi-sig. The Engine does not choose, it refuses: every `allocate()` call is checked against a cap per pool, a cap per originator, a cap per jurisdiction and a reserve floor, and any one of them failing reverts the whole call. All four are shares of net assets in basis points, and the floor is 2500 bps on testnet, enforced by the Engine and again by the Vault against its own book. In V2 an off-chain optimizer proposes allocations through the same admin-gated functions, with the same enforcement.
